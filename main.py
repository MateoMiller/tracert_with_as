...
def main():
    tracert = get_tracert_output(sys.argv[1])
    #tracert = open('yandex.ru.txt', 'r').read()

    #Получаем ip всех промежуточных звеньев
    infos = parse_info_from_tracert(tracert)

    #У частных адресов нельзя узнать AS и другую информацию
...
def get_tracert_output(ip_address):
    #используем встроенную утилиту tracert
...
def parse_info_from_tracert(tracert):
    #Несколько линий в утилите tracert являются вспомогательными, так что их лучше удалить, чтобы они не мешались
